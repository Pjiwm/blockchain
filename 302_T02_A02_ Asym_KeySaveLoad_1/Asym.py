...
def save_keys(keys_file_name, keys_list):
    ...
    pem_list = []
    pem_list = [(key_set[0],
                key_set[1].private_bytes(
                    encoding=serialization.Encoding.PEM,
                    format=serialization.PrivateFormat.TraditionalOpenSSL,
                    encryption_algorithm=serialization.NoEncryption()
    ),
        key_set[2].public_bytes(encoding=serialization.Encoding.PEM,
                                format=serialization.PublicFormat.SubjectPublicKeyInfo
                                )
    )
        for key_set in keys_list]
    pickle.dump(pem_list, file, -1)
    file.close()
...
def load_keys(keys_file_name):
    ...
    return [(key_set[0],
             serialization.load_pem_private_key(
                 key_set[1], None, default_backend()),
             serialization.load_pem_public_key(key_set[2], default_backend()))
            for key_set in file_load]
