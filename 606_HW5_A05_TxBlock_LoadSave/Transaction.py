...
class Tx:
    ...
    def __repr__(self):
        ...
        for input in self.inputs:
            return_string += f"{input[1]} from {input[0]}\n"
        return_string += "OUTPUTS:\n"
        for output in self.outputs:
            return_string += f"{output[1]} to {output[0]}\n"
        if self.reqd: return_string += "EXTRA REQUIRED_SIGNATURES:"
        for reqd in self.reqd:
            return_string += f"{reqd}\n"
        return return_string + "END"
